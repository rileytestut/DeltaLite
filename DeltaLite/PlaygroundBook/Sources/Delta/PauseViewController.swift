...
extension PauseViewController
{
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?)
    {
        ...
        case "saveStates":
            ...
            saveStatesViewController.game = self.emulatorCore?.game as? NESGame
        ...
        case "cheats":
            ...
            cheatsViewController.game = self.emulatorCore?.game as? NESGame
...
private extension PauseViewController
{
    func updatePauseItems()
    {
        ...
        self.saveStateItem = MenuItem(text: NSLocalizedString("Save State", comment: ""), image: UIImage(named: "SaveSaveState")?.withRenderingMode(.alwaysTemplate), action: { [unowned self] _ in
            self.saveStatesViewControllerMode = .saving
            self.performSegue(withIdentifier: "saveStates", sender: self)
        })
        
        self.loadStateItem = MenuItem(text: NSLocalizedString("Load State", comment: ""), image: UIImage(named: "LoadSaveState")?.withRenderingMode(.alwaysTemplate), action: { [unowned self] _ in
            self.saveStatesViewControllerMode = .loading
            self.performSegue(withIdentifier: "saveStates", sender: self)
        })
        
        self.cheatCodesItem = MenuItem(text: NSLocalizedString("Cheat Codes", comment: ""), image: UIImage(named: "CheatCodes")?.withRenderingMode(.alwaysTemplate), action: { [unowned self] _ in
