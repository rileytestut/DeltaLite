...
class SaveStatesViewController: UICollectionViewController
{
    ...
    private let dataSource: RSTFetchedResultsCollectionViewPrefetchingDataSource<NESSaveState, UIImage>
    ...
    {
        self.dataSource = RSTFetchedResultsCollectionViewPrefetchingDataSource<NESSaveState, UIImage>(fetchedResultsController: NSFetchedResultsController())
...
private extension SaveStatesViewController
{
    ...
    {
        let fetchRequest: NSFetchRequest<NESSaveState> = NESSaveState.fetchRequest()
        fetchRequest.returnsObjectsAsFaults = false
        fetchRequest.predicate = NSPredicate(format: "%K == %@", #keyPath(NESSaveState.game), self.game)
        fetchRequest.sortDescriptors = [NSSortDescriptor(key: #keyPath(NESSaveState.type), ascending: true), NSSortDescriptor(key: #keyPath(NESSaveState.creationDate), ascending: true)]
        
        self.dataSource.fetchedResultsController = NSFetchedResultsController(fetchRequest: fetchRequest, managedObjectContext: DatabaseManager.shared.viewContext, sectionNameKeyPath: #keyPath(NESSaveState.type), cacheName: nil)
    }
    ...
    {
        var saveState: NESSaveState!
        
        let backgroundContext = DatabaseManager.shared.newBackgroundContext()
        backgroundContext.performAndWait {
            let game = backgroundContext.object(with: self.game.objectID) as! NESGame
            
            saveState = NESSaveState.insertIntoManagedObjectContext(backgroundContext)
    ...
    func updateSaveState(_ saveState: NESSaveState)
    {
    ...
    func deleteSaveState(_ saveState: NESSaveState)
    {
    ...
    func renameSaveState(_ saveState: NESSaveState)
    {
    ...
    func rename(_ saveState: NESSaveState, with name: String?)
    {
        var name = name
        if (name ?? "").count == 0
        {
        ...
        DatabaseManager.shared.performBackgroundTask { (context) in
            let saveState = context.object(with: saveState.objectID) as! NESSaveState
            saveState.name = name
    ...
    func updatePreviewSaveState(_ saveState: NESSaveState?)
    {
        ...
        alertController.addAction(UIAlertAction(title: NSLocalizedString("Change", comment: ""), style: .default, handler: { (action) in
            ...
            DatabaseManager.shared.performBackgroundTask { (context) in
                let game = context.object(with: self.game.objectID) as! NESGame
                
                if let saveState = saveState
                {
                    let previewSaveState = context.object(with: saveState.objectID) as! NESSaveState
    ...
    func lockSaveState(_ saveState: NESSaveState)
    {
        let backgroundContext = DatabaseManager.shared.newBackgroundContext()
        backgroundContext.performAndWait() {
            let temporarySaveState = backgroundContext.object(with: saveState.objectID) as! NESSaveState
            temporarySaveState.type = .locked
            backgroundContext.saveWithErrorLogging()
        }
    }
    
    func unlockSaveState(_ saveState: NESSaveState)
    {
        let backgroundContext = DatabaseManager.shared.newBackgroundContext()
        backgroundContext.performAndWait() {
            let temporarySaveState = backgroundContext.object(with: saveState.objectID) as! NESSaveState
    ...
    func actionsForSaveState(_ saveState: NESSaveState) -> [Action]?
    {
