...
{
    func activateCheatWithErrorLogging(_ cheat: NESCheat)
    {
        do
        {
    ...
    func rst_updateCheats()
    {
        ...
        backgroundContext.performAndWait {
            ...
            let predicate = NSPredicate(format: "%K == %@", #keyPath(NESCheat.game), game)
            
            let cheats = NESCheat.instancesWithPredicate(predicate, inManagedObjectContext: backgroundContext, type: NESCheat.self)
            for cheat in cheats
            {
                if cheat.isEnabled
                {
