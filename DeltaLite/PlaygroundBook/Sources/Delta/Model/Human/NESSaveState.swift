...
@objc(NESSaveState)
public class NESSaveState: _SaveState, SaveStateProtocol
{
    ...
    public override func prepareForDeletion()
    {
        ...
        guard !managedObjectContext.insertedObjects.contains(where: { ($0 as? NESSaveState)?.identifier == self.identifier }) else { return }
    ...
    class func fetchRequest(for game: NESGame, type: SaveStateType) -> NSFetchRequest<NESSaveState>
    {
        let predicate = NSPredicate(format: "%K == %@ AND %K == %d", #keyPath(NESSaveState.game), game, #keyPath(NESSaveState.type), type.rawValue)
